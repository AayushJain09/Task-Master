...
3. **Quick-add reminders** – POST to `/api/reminders/quick-add` with natural language input plus a timezone override to confirm the parser honors the locale.
4. **Task dashboards** – hit `/api/tasks`, `/api/tasks/status/:status`, and `/api/dashboard/metrics` with the `timezone` query parameter to ensure due-date filters and aggregates shift correctly.
5. **Sync workflow** – submit a `/api/reminders/sync` payload that includes the top-level `timezone` field and verify conflicts/serverChanges are localized for that timezone.
